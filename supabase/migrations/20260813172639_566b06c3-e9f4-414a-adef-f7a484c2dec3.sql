INSERT INTO public.site_settings (key, value) VALUES ('ploomes:default_filial_id', NULL)
...
BEGIN
  ...
  IF v_filial IS NULL THEN
    BEGIN
      SELECT NULLIF(value,'')::bigint INTO v_filial
      FROM public.site_settings WHERE key = 'ploomes:default_filial_id';
    EXCEPTION WHEN OTHERS THEN v_filial := NULL;
    END;
  END IF;

  IF v_filial IS NULL THEN
    v_filial := public.ploomes_filial_id('londrina'::public.unit_enum);
  END IF;

  IF v_unit IS NULL THEN
    INSERT INTO public.integration_sync_log (source, action, status, message, payload)
    VALUES ('ploomes_form','create_lead','info','Cidade sem filial mapeada, usando filial padrão: '||coalesce(NEW.cidade,'—'), jsonb_build_object('lead_id', NEW.id, 'cidade', NEW.cidade, 'filial', v_filial));
  END IF;
