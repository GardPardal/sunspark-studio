...
const COMMANDS: Command[] = [
  ...
  { id: "c-financeiro", label: "Financeiro", to: "/mod/financeiro", keywords: ["financeiro", "cac", "roas", "receita"] },
  { id: "c-liz", label: "Abrir LIZ Studio (imagens)", to: "/liz-studio", keywords: ["liz", "imagem", "midjourney", "criativo"] },
  { id: "c-ia", label: "Insights da IA", to: "/mod/ia", keywords: ["ia", "insights", "recomendacoes"] },
  { id: "c-saude", label: "Saúde do sistema", to: "/mod/saude", keywords: ["saude", "status", "integracao", "health"] },
  ...
  { id: "c-inventario", label: "Inventário de estoque", to: "/inventario", keywords: ["inventario", "estoque", "almoxarifado", "material", "saldo"] },
];
...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const filteredCommands = useMemo(() => {
    ...
    return COMMANDS.filter(
      (c) =>
        ...
        c.keywords.some((k) => k.includes(t)),
    );
  }, [q]);
  ...
  const go = (idx: number) => {
    ...
    if (it.kind === "cmd") {
      ...
      else if (h.entity_type === "campaign" || h.entity_type === "creative") navigate({ to: "/mod/marketing" });
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-border/60 px-4 py-3">
          ...
          />
          {loading && <span className="text-[10px] uppercase tracking-wide text-muted-foreground">buscando…</span>}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          ...
          {items.map((it, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    {it.cmd.hint && <span className="block truncate text-[11px] text-muted-foreground">{it.cmd.hint}</span>}
                  </span>
            ...
            return (
              ...
              >
                <span className={cn("grid h-8 w-8 shrink-0 place-items-center rounded-lg text-[10px] font-semibold uppercase", tone)}>
