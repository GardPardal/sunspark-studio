    ...
    <Tabs defaultValue="orcamento" className="w-full">
      <TabsList>
        <TabsTrigger value="orcamento">
          <Dices className="h-3.5 w-3.5 mr-1" /> Orçamento
        </TabsTrigger>
        <TabsTrigger value="visita_tecnica">
          <Wrench className="h-3.5 w-3.5 mr-1" /> Visita técnica
        </TabsTrigger>
      </TabsList>
      <TabsContent value="orcamento" className="mt-4">
        <SpinCard mode="orcamento" />
      </TabsContent>
      <TabsContent value="visita_tecnica" className="mt-4">
        <SpinCard mode="visita_tecnica" />
      </TabsContent>
    </Tabs>
  );
}
...
function SpinCard({ mode }: { mode: Mode }) {
  ...
  const spinM = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        `${res.distributed} lead(s) distribuído(s) para ${label}. Pode reatribuir manualmente no Kanban se precisar.`,
      );
  ...
  const queueCount =
    mode === "orcamento" ? (queueQ.data?.count ?? 0) : (queueQ.data?.visitaCount ?? 0);
    ...
    <Card className="p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-lg font-semibold">
          {isVisita
            ? "Distribuição de visitas técnico-comerciais"
            : "Roleta SDR — distribuição de orçamentos"}
        </h3>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Card className="p-3 bg-secondary/30">
          ...
          {(queueQ.data?.semCidade ?? 0) > 0 && (
            <div className="mt-2 text-xs text-amber-600">
              ⚠ {queueQ.data?.semCidade} lead(s) sem cidade mapeada — não entram em nenhuma unidade
              até o cadastro da cidade.
            </div>
          )}
        </Card>
        ...
        <Card className="p-3 bg-secondary/30">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-1 flex items-center gap-1">
            <Users2 className="h-3 w-3" /> Consultores ativos (
            {UNITS.find((u) => u.key === unit)?.label})
          </div>
