...
export const getLeadsDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: out, error } = await context.supabase.rpc(
      "leads_dashboard" as never,
      { _days: data.days ?? 30 } as never,
    );
...
export const listLeads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.q?.trim()) {
      ...
      q =
        digits.length >= 4
          ? q.or(`telefone_e164.ilike.%${digits}%,telefone.ilike.%${digits}%,nome.ilike.%${t}%`)
          : q.or(
              `nome.ilike.%${t}%,email.ilike.%${t}%,cidade.ilike.%${t}%,id.eq.${/^[0-9a-f-]{36}$/i.test(t) ? t : "00000000-0000-0000-0000-000000000000"}`,
            );
    }
    ...
    if (data.sync)
      q =
        data.sync === "erro"
          ? q.in("ploomes_sync_status", ["erro", "revisao_manual"])
          : q.eq("ploomes_sync_status", data.sync);
    ...
    const sort = [
      ...
      : "created_at";
    q = q
      .order(sort, { ascending: data.dir === "asc", nullsFirst: false })
...
export const getLeadDetail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [events, queue, dups] = await Promise.all([
      ...
      supabaseAdmin
        .from("leads")
        .select("id,nome,created_at,origem_principal")
        .eq("duplicado_de", data.id),
    ]);
    let messages: any[] = [];
    ...
    if (convId) {
      const { data: m } = await supabaseAdmin
        .from("wa_messages")
        .select("id,direction,body,msg_type,media_url,ai_generated,occurred_at,status")
        .eq("conversation_id", convId)
        .order("occurred_at", { ascending: true })
        .limit(200);
      messages = m ?? [];
    }
    const { data: timeline } = await (supabaseAdmin as any)
...
export const retryLeadSync = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const r = JSON.parse(JSON.stringify(r0)) as typeof r0 extends infer T
      ? T extends { plan?: unknown }
        ? Omit<T, "plan"> & { plan?: Record<string, string | number | boolean | null> }
        : T
      : never;
    if (!data.dryRun) {
      await (supabaseAdmin as any)
        ...
        .update(
          r.ok
            ? { status: "sincronizado", last_error: null }
            : { status: r.retry ? "pendente" : "revisao_manual", last_error: r.error },
        )
        .eq("lead_id", data.id)
      ...
      if (!r.ok)
        await supabaseAdmin
          .from("leads")
          .update({
            ploomes_sync_status: r.retry ? "erro" : "revisao_manual",
            ploomes_sync_error: r.error,
          } as never)
          .eq("id", data.id);
    }
    return r;
...
export const traceLead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const leadsQ = isId
      ? supabaseAdmin.from("leads").select("*").eq("id", t)
      : supabaseAdmin
          .from("leads")
          .select("*")
    ...
    const contactsQ = phone
      ? supabaseAdmin
          .from("wa_contacts")
          .select("id,phone_e164,profile_name,lead_id,created_at,last_inbound_at")
          .eq("phone_e164", phone)
      : supabaseAdmin
          .from("wa_contacts")
          .select("id,phone_e164,profile_name,lead_id,created_at,last_inbound_at")
    ...
    const { data: convs } = contactIds.length
      ? await supabaseAdmin
          .from("wa_conversations")
          .select("id,contact_id,status,last_message_at,created_at,lead_id")
    ...
    const { data: events } = leadIds.length
      ? await (supabaseAdmin as any)
          ...
          .limit(100)
      : { data: [] };
    const { data: queue } = leadIds.length
      ? await (supabaseAdmin as any)
          .from("lead_sync_queue")
          .select("*")
          .in("lead_id", leadIds)
    ...
    const { data: lastMsgs } = convIds.length
      ? await supabaseAdmin
          .from("wa_messages")
          .select("id,conversation_id,direction,body,occurred_at,ai_generated")
          .in("conversation_id", convIds)
          .order("occurred_at", { ascending: false })
          .limit(15)
      : { data: [] as any[] };
    const { data: wa_events } = phone
      ? await supabaseAdmin
          .from("wa_events")
          .select("id,provider_event_id,event_kind,process_status,received_at,error")
          .order("received_at", { ascending: false })
          .limit(300)
      : { data: [] as any[] };
    const digits = phone?.replace(/\D/g, "") ?? "";
    const matchingEvents = (wa_events ?? [])
      .filter((e: any) => digits && JSON.stringify(e).includes(digits.slice(-8)))
      .slice(0, 10);
    return {
      phone,
      leads: leads ?? [],
...
export const saveLeadRules = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { minFatura: number; exigirFatura: boolean; defaultOwnerId: number | null }) => d,
  )
  .handler(async ({ context, data }) => {
    ...
    const rows = [
      ...
      {
        key: "ploomes:default_owner_id",
        value: data.defaultOwnerId ? String(data.defaultOwnerId) : "",
      },
    ];
    for (const r of rows) {
...
export const getDuplicateReports = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    await assertManager(context);
    const { internalDuplicateReport, ploomesDuplicateReport } =
      await import("@/lib/leads/reconcile.server");
    const [internal, ploomes] = await Promise.all([
      internalDuplicateReport(200),
      ploomesDuplicateReport(data.sinceISO ?? "2026-08-01T00:00:00Z"),
    ]);
...
export const listQueue = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = (supabaseAdmin as any)
      .from("lead_sync_queue")
      .select("*, leads(nome,telefone_e164,origem_principal)")
