...
export async function listLizConversations(
  ...
  for (const c of data ?? []) {
    ...
    const { count: ai } = await supabaseAdmin
      .from("wa_messages")
      .select("id", { count: "exact", head: true })
      .eq("conversation_id", c.id)
      .eq("ai_generated", true);
    if (!ai) continue;
    const { count: inb } = await supabaseAdmin
      .from("wa_messages")
      .select("id", { count: "exact", head: true })
      .eq("conversation_id", c.id)
    ...
    out.push({
      ...
      phone: ct?.phone_e164 ?? null,
      profile_name: ct?.profile_name ?? null,
      lead_id: (c as any).lead_id ?? ct?.lead_id ?? null,
    });
...
export async function reconcileLizLeads(opts: {
  dryRun: boolean;
  limit?: number;
  offset?: number;
  syncPloomes?: boolean;
  ...
  for (const c of convs) {
    ...
    if (!phone) {
      rows.push({
        ...
        contact_id: c.contact_id,
        phone: c.phone,
        nome_perfil: c.profile_name,
        lead_id: c.lead_id,
        acao: "ignorar",
        status_previsto: "—",
        campos_preenchidos: [],
        pendentes: ["Telefone inválido"],
        ploomes: "—",
      });
      totals.ignorar++;
      continue;
    ...
    try {
      if (!opts.dryRun) {
        const r = await qualifyLeadFromConversation({
          conversationId: c.id,
        ...
        if ("skipped" in r) {
          rows.push({
            ...
            lead_id: c.lead_id,
            acao: "sem_dados",
            status_previsto: r.skipped ?? "sem dados",
            campos_preenchidos: [],
            pendentes: [],
            ploomes: "—",
          });
        ...
        rows.push({
          ...
          nome_perfil: c.profile_name,
          lead_id: r.leadId,
          acao: r.created ? "criar" : "atualizar",
          status_previsto: r.qualification.status,
          campos_preenchidos: Object.entries(r.extraction)
            .filter(([k, v]) => v != null && v !== false && !["confianca"].includes(k))
            .map(([k]) => k),
          pendentes: r.qualification.pendentes,
          ploomes,
        });
        if (r.created) totals.criar++;
        else totals.atualizar++;
        continue;
      }
      ...
      const { data: msgs } = await supabaseAdmin
        ...
        .limit(80);
      const ex = await extractFromConversation(
        (msgs ?? []).map((m: any) => ({
      ...
      if (!ex) {
        rows.push({
          ...
          lead_id: c.lead_id,
          acao: "sem_dados",
          status_previsto: "extração falhou",
          campos_preenchidos: [],
          pendentes: [],
          ploomes: "—",
        });
      ...
      const q = computeQualification({ id: existing?.id ?? "novo", ...merged } as any, rules, {
        recusou: ex.recusou_informar,
        humano: ex.pediu_humano,
        semInteresse: ex.sem_interesse,
        interesse: ex.interesse,
      });
      let ploomes = existing?.ploomes_deal_id
        ? `já vinculado (negócio ${existing.ploomes_deal_id})`
        : "—";
      if (!existing?.ploomes_deal_id && (q.status === "qualificado" || q.status === "humano")) {
        const f = await findPloomesContacts({
          telefone_e164: phone,
          email: ex.email,
          cpf_cnpj: ex.cpf_cnpj,
        });
        ploomes = f.primary
          ? `reutilizaria contato ${f.primary.Id}${f.all.length > 1 ? ` (${f.all.length - 1} duplicado(s))` : ""}`
          : "criaria contato + negócio";
      }
      rows.push({
        conversation_id: c.id,
        contact_id: c.contact_id,
        ...
        status_previsto: q.status,
        campos_preenchidos: Object.entries(ex)
          .filter(([k, v]) => v != null && v !== false && !["confianca"].includes(k))
          .map(([k]) => k),
        pendentes: q.pendentes,
        ploomes,
      });
      if (existing) totals.atualizar++;
      else totals.criar++;
    } catch (e) {
      totals.erros++;
      rows.push({
...
export async function internalDuplicateReport(limit = 100) {
  const { data } = await supabaseAdmin
    .from("leads")
    .select(
      "id, nome, telefone_e164, origem_principal, origem, created_at, ploomes_deal_id, ploomes_contact_id, stage, duplicado_de",
    )
...
export async function consolidateInternalDuplicates(opts: { dryRun: boolean; limit?: number }) {
  ...
  for (const g of groups) {
    const sorted = [...g.leads].sort(
      (a, b) =>
        Number(Boolean(b.ploomes_deal_id)) - Number(Boolean(a.ploomes_deal_id)) ||
    ...
    const { data: full } = await supabaseAdmin
      .from("leads")
      .select("*")
      .in("id", [keep.id, ...others.map((o) => o.id)]);
    ...
    for (const o of (full ?? []).filter((r: any) => r.id !== keep.id) as any[]) {
      for (const k of [
        ...
        if ((keepRow[k] == null || keepRow[k] === "") && o[k] != null && patch[k] === undefined)
          patch[k] = o[k];
      }
    ...
    await supabaseAdmin
      .from("leads")
      .update({ duplicado_de: keep.id } as never)
      .in(
        "id",
        others.map((o) => o.id),
      );
    await (supabaseAdmin as any).from("lead_events").insert({
      lead_id: keep.id,
      phone_masked: g.phone.replace(/\d(?=\d{4})/g, "*"),
      event: "lead.merged",
      step: "consolidacao",
      result: `${others.length} duplicado(s) marcados`,
      detail: { marked: others.map((o) => o.id), patch: Object.keys(patch) },
    });
