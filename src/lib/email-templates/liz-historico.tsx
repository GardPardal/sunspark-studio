...
const Email = ({
  ...
  <Html lang="pt-BR">
    <Head />
    <Preview>Conversa da LIZ — {user_name || user_email || session_id}</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={card}>
          ...
          <Text style={value}>
            {user_name || "(anônimo)"} {user_email ? `· ${user_email}` : ""}
          </Text>
          <Hr style={hr} />
          <Text style={label}>Modo</Text>
          <Text style={value}>{mode === "internal" ? "Painel interno" : "Landing / captura"}</Text>
          <Hr style={hr} />
          <Text style={label}>Sessão</Text>
          <Text style={value}>{session_id}</Text>
          <Hr style={hr} />
          <Text style={label}>Início</Text>
          <Text style={value}>{fmt(first_at)}</Text>
          <Hr style={hr} />
          <Text style={label}>Última mensagem</Text>
          <Text style={value}>{fmt(updated_at)}</Text>
          <Hr style={hr} />
          <Text style={label}>Total de mensagens</Text>
          <Text style={value}>{message_count}</Text>
          {page_url && (
            <>
              <Hr style={hr} />
              <Text style={label}>Página</Text>
...
export const template = {
  ...
  subject: (d: Record<string, any>) =>
    `💬 Conversa LIZ — ${d.user_name || d.user_email || d.session_id || "anônimo"}`,
