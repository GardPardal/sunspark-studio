  ...
  <Html lang="pt-BR" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={lead}>
          Olá {consultor}, um novo lead foi atribuído a você. Confirme o atendimento em até{" "}
          <strong>2 horas úteis</strong>, senão ele volta para a fila.
        </Text>

        <Section style={card}>
          <Text style={label}>Nome</Text>
          <Text style={value}>{lead_nome}</Text>
          <Hr style={hr} />
          <Text style={label}>Telefone</Text>
          <Text style={value}>{lead_telefone}</Text>
          <Hr style={hr} />
          <Text style={label}>Cidade</Text>
          <Text style={value}>
            {lead_cidade}
            {lead_estado ? `/${lead_estado}` : ""}
          </Text>
          <Hr style={hr} />
          <Text style={label}>Conta de luz média</Text>
          <Text style={value}>{lead_valor_conta}</Text>
          <Hr style={hr} />
          <Text style={label}>Prazo de confirmação</Text>
          <Text style={valueBold}>{formatBR(deadline)}</Text>
        </Section>

        <Section style={{ textAlign: "center", margin: "24px 0" }}>
          <Button href={cta_url} style={btn}>
            Abrir lead no CRM
          </Button>
        </Section>

        <Text style={foot}>
          LZ7 Energia — este email foi disparado automaticamente pela roleta SDR.
        </Text>
      </Container>
    </Body>
  </Html>
...
export const template = {
  ...
  previewData: {
    ...
    lead_telefone: "43 99999-0000",
    lead_cidade: "Londrina",
    lead_estado: "PR",
    lead_valor_conta: "R$ 850",
