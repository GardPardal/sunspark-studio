...
import {
  generateInsights,
  getInsightsBundlePreview,
  type InsightResponse,
} from "@/modules/ia/insights.functions";
...
export const Route = createFileRoute("/_authenticated/mod/ia")({
  head: () => ({
    meta: [{ title: "IA — Motor de Insights" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: IAModule,
});
...
function IAModule() {
  ...
  return (
    <ModuleShell
      title="Motor de IA"
      subtitle="Insights baseados em dados reais dos últimos 30 dias"
      active="ia"
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              A IA lê seus dados reais (não invente) e devolve o que fazer, por quê e o impacto
              estimado.
              {pv.data && (
                <>
                  {" "}
                  Bundle atual: <b>{pv.data.leadsCount}</b> leads · <b>{pv.data.metaRows}</b> linhas
                  Meta desde {pv.data.periodo}.
                </>
              )}
            </p>
          </div>
          <Button onClick={() => m.mutate()} disabled={m.isPending}>
            {m.isPending ? (
              ...
              <>
                <Sparkles className="h-4 w-4 mr-1.5" /> Gerar insights
              </>
            )}
          </Button>
        </div>
      ...
      {m.error && (
        <Card className="p-5 text-red-600 text-sm">Erro: {(m.error as Error).message}</Card>
      )}
            ...
            {m.data.insights.map((i, idx) => (
              <Card key={idx} className="p-4 space-y-2">
                ...
                <div className="text-sm text-muted-foreground">
                  <b>Por quê:</b> {i.por_que}
                </div>
                <div className="text-sm">
                  <b>Impacto:</b> {i.impacto}
                </div>
                ...
                {i.ganho_estimado_brl != null && i.ganho_estimado_brl > 0 && (
                  <div className="text-xs text-muted-foreground">
                    Ganho estimado: <b>{brl(i.ganho_estimado_brl)}</b>
                  </div>
                )}
              </Card>
            ))}
