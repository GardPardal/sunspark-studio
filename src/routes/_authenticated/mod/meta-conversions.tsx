...
function MetaConversionsPage() {
  ...
  return (
    <ModuleShell
      title="Log de Conversões Meta"
      subtitle="Todos os eventos enviados para a Meta CAPI"
      active="meta-conversions"
    >
      {/* Métricas do dia */}
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
        ...
        <MetricCard
          label="Erros"
          value={m.errors ?? 0}
          tone={(m.errors ?? 0) > 0 ? "text-red-700" : ""}
          hint={m.skipped ? `${m.skipped} bloqueados por validação` : undefined}
        />
        <MetricCard
          label="Match Quality média"
          value={m.avgMatch ? `${m.avgMatch}/10` : "—"}
          tone={
            m.avgMatch >= 7
              ? "text-emerald-700"
              : m.avgMatch >= 4
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
        <div className="text-sm text-muted-foreground">
          {rows.length} eventos listados · atualiza em tempo real
        </div>
        <div className="flex gap-2">
          <Button
            size="sm"
            variant="outline"
            onClick={() => qc.invalidateQueries({ queryKey: ["meta-conv"] })}
          >
                  ...
                  <tr key={r.id} className="border-t hover:bg-muted/30">
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      ...
                      <Button
                        ...
                        title={
                          r.status_detail === "skipped_validation"
                            ? "Sem payload — evento bloqueado por validação"
                            : "Reenviar"
                        }
                      >
        ...
        <Card className="p-4">
          ...
          {!selectedId && (
            <p className="text-sm text-muted-foreground">
              Selecione um evento para ver payload, resposta e motivos de validação.
            </p>
          )}
          {selectedId &&
            detQ.data &&
            (detQ.data as any).event &&
            (() => {
              ...
              return (
                <div className="space-y-3 text-xs">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {ev.match_quality != null && (
                      <Badge variant="outline">Match {ev.match_quality}/10</Badge>
                    )}
                  </div>
                  <div>
                    <b>fbtrace_id:</b> <code>{ev.fbtrace_id || "—"}</code>
                  </div>
                  <div>
                    <b>event_id:</b> <code>{ev.event_id || "—"}</code>
                  </div>
                  {ev.validation_errors?.length ? (
                    <div className="rounded border border-amber-500/40 bg-amber-500/5 p-2">
                      ...
                      <ul className="list-disc pl-4 mt-1">
                        {ev.validation_errors.map((e: string, i: number) => (
                          <li key={i}>{e}</li>
                        ))}
                      </ul>
                    </div>
