  ...
  head: () => ({
    meta: [
      { title: "Central de Leads — Solar OS" },
      {
        name: "description",
        content:
          "Fonte oficial de leads: entrada, qualificação, fila e sincronização com o Ploomes.",
      },
...
function LeadsPage() {
  ...
  return (
    <ModuleShell
      ...
      subtitle="Fonte oficial: entrada → validação → qualificação → fila → Ploomes"
    >
      ...
      <div className="mt-4">
        {tab === "painel" && (
          <Painel onOpen={(id) => setSelected(id)} onFilter={() => setTab("leads")} />
        )}
...
function Stat({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "text-left rounded-xl border border-border bg-card p-3 hover:bg-accent/40 transition-colors",
...
function Painel({ onOpen, onFilter }: { onOpen: (id: string) => void; onFilter: () => void }) {
  ...
  const recent = useQuery({
    ...
    queryFn: () => listFn({ data: { pageSize: 8 } }),
  });
  const d = q.data;
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm">
        ...
        {[7, 30, 90, 365].map((n) => (
          <button
            ...
            className={cn(
              ...
              days === n ? "bg-primary text-primary-foreground border-primary" : "border-border",
            )}
          >
            {n}d
          </button>
        ...
        <span className="ml-auto text-xs text-muted-foreground">
          Atualizado {fmtDate(d.atualizado_em)}
        </span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        <Stat
          label="Em qualificação / pendências"
          value={d.em_qualificacao + d.com_pendencias}
          intent="warning"
        />
        ...
        <Stat
          label="Erros / revisão"
          value={d.ploomes_erro}
          intent={d.ploomes_erro ? "danger" : undefined}
        />
      ...
      <DsCard>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div>
            ...
            <div className="font-semibold">{fila.pendente ?? 0}</div>
          </div>
          <div>
            <span className="text-muted-foreground">Processando</span>
            <div className="font-semibold">{fila.processando ?? 0}</div>
          </div>
          <div>
            <span className="text-muted-foreground">Revisão manual</span>
            <div className={cn("font-semibold", fila.revisao_manual ? "text-warning" : "")}>
              {fila.revisao_manual ?? 0}
            </div>
          </div>
          <div>
            <span className="text-muted-foreground">Último sucesso</span>
            <div className="font-semibold">{fmtDate(fila.ultimo_sucesso)}</div>
          </div>
        </div>
        {fila.mais_antigo_pendente && (
          <p className="text-xs text-muted-foreground mt-2">
            Item mais antigo aguardando desde {fmtDate(fila.mais_antigo_pendente)}
          </p>
        )}
        {fila.ultimo_erro && (
          <p className="text-xs text-destructive mt-1 break-all">Último erro: {fila.ultimo_erro}</p>
        )}
      </DsCard>
      ...
      <div className="grid md:grid-cols-3 gap-3">
        {[
          ...
          [
            "Por status",
            (d.por_status ?? []).map((x: any) => ({ ...x, k: STATUS_LABEL[x.k] ?? x.k })),
          ],
            ...
            >
              <td className="px-2 py-2">
                <div className="font-medium">
                  {l.nome || <span className="text-muted-foreground">Não informado</span>}
                </div>
                {l.qualificado_por === "liz" && (
                  <span className="text-[11px] text-primary inline-flex items-center gap-1">
                    <Sparkles className="h-3 w-3" /> Liz
                  </span>
                )}
              </td>
              <td className="px-2 py-2 tabular-nums">
                {fmtPhone(l.telefone_e164) !== "—" ? fmtPhone(l.telefone_e164) : l.telefone || "—"}
              </td>
              <td className="px-2 py-2">
                {l.cidade || <span className="text-muted-foreground">—</span>}
              </td>
              <td className="px-2 py-2">{money(l.valor_conta_num ?? null)}</td>
              <td className="px-2 py-2">
                <span className="text-xs">{l.origem_principal || l.origem || "—"}</span>
              </td>
              <td className="px-2 py-2">
                <DsBadge intent={STATUS_INTENT[l.qualificacao_status] ?? "neutral"} size="sm">
                  {STATUS_LABEL[l.qualificacao_status] ?? l.qualificacao_status}
                </DsBadge>
                {Array.isArray(l.campos_pendentes) && l.campos_pendentes.length > 0 && (
                  <div className="text-[11px] text-muted-foreground mt-0.5">
                    {l.campos_pendentes.length} pendência(s)
                  </div>
                )}
              </td>
              <td className="px-2 py-2">
                <DsBadge intent={SYNC_INTENT[l.ploomes_sync_status] ?? "neutral"} size="sm">
                  {SYNC_LABEL[l.ploomes_sync_status] ?? l.ploomes_sync_status}
                </DsBadge>
                {l.ploomes_deal_id && (
                  <div className="text-[11px] text-muted-foreground">#{l.ploomes_deal_id}</div>
                )}
              </td>
...
function LeadsList({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input
            ...
            placeholder="Nome, telefone, e-mail, cidade ou ID"
            className="pl-8"
          />
        </div>
        <select
        ...
        >
          ...
          {Object.entries(SYNC_LABEL)
            .filter(([k]) => k !== "revisao_manual")
            .map(([k, v]) => (
              <option key={k} value={k}>
                {v}
              </option>
      ...
      {query.isLoading ? (
        <DsSkeletonList rows={8} />
      ) : (
        <LeadTable rows={query.data?.rows ?? []} onOpen={onOpen} />
      )}
...
function Fila({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const m = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        `Fila processada: ${r.ok} ok, ${r.retry} para retentar, ${r.manual} revisão manual`,
      );
      qc.invalidateQueries({ queryKey: ["leads"] });
    },
        ...
        {["", "pendente", "processando", "sincronizado", "revisao_manual"].map((s) => (
          <button
            ...
            className={cn(
              ...
              status === s ? "bg-primary text-primary-foreground border-primary" : "border-border",
            )}
          >
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="font-medium text-sm">
                    {it.leads?.nome || "Não informado"}{" "}
                    <span className="text-muted-foreground font-normal">
                      · {fmtPhone(it.leads?.telefone_e164)}
                    </span>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Tentativas {it.attempts}/{it.max_attempts} · próxima{" "}
                    {fmtDate(it.next_attempt_at)} · origem {it.leads?.origem_principal ?? "—"}
                  </div>
                  {it.last_error && (
                    <div className="text-xs text-destructive mt-1 break-all">{it.last_error}</div>
                  )}
                </div>
                <DsBadge intent={SYNC_INTENT[it.status] ?? "neutral"} size="sm">
                  {SYNC_LABEL[it.status] ?? it.status}
                </DsBadge>
              </div>
            </DsCard>
          ))}
...
function Rastreio({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const query = useQuery({
    queryKey: ["leads", "trace", term],
    queryFn: () => fn({ data: { q: term } }),
    ...
    <div className="space-y-3">
      ...
      >
        <Input
          ...
          placeholder="Telefone (qualquer formato) ou ID do lead"
        />
      ...
      {d && (
        <div className="grid md:grid-cols-2 gap-3">
          <DsCard>
            ...
            {d.leads.length === 0 && (
              <p className="text-sm text-muted-foreground">Nenhum lead com este telefone.</p>
            )}
            {d.leads.map((l: any) => (
              <button
                key={l.id}
                onClick={() => onOpen(l.id)}
                className="block w-full text-left text-sm border-t border-border py-2 hover:bg-accent/40"
              >
                <div className="font-medium">
                  {l.nome || "Não informado"}{" "}
                  {l.duplicado_de && (
                    <DsBadge intent="warning" size="sm">
                      duplicado
                    </DsBadge>
                  )}
                </div>
                <div className="text-xs text-muted-foreground">
                  {l.origem_principal ?? l.origem} ·{" "}
                  {STATUS_LABEL[l.qualificacao_status] ?? l.qualificacao_status} ·{" "}
                  {SYNC_LABEL[l.ploomes_sync_status] ?? l.ploomes_sync_status} ·{" "}
                  {fmtDate(l.created_at)}
                </div>
          ...
          <DsCard>
            <DsCardHeader
              title={`WhatsApp: ${d.contacts.length} contato(s), ${d.conversations.length} conversa(s)`}
            />
            {d.contacts.map((c: any) => (
              <div key={c.id} className="text-sm border-t border-border py-2">
                ...
                <div className="text-xs text-muted-foreground">
                  último recebido {fmtDate(c.last_inbound_at)} · lead{" "}
                  {c.lead_id ? "vinculado" : "não vinculado"}
                </div>
              </div>
            ))}
            {d.lastMessages.slice(0, 5).map((m: any) => (
              <div key={m.id} className="text-xs border-t border-border py-1.5">
                <span className="text-muted-foreground">
                  {fmtDate(m.occurred_at)} ·{" "}
                  {m.direction === "inbound" ? "Cliente" : m.ai_generated ? "Liz" : "Atendente"}:
                </span>{" "}
                {m.body?.slice(0, 120)}
              </div>
            ))}
          ...
          <DsCard>
            ...
            {d.webhookEvents.length === 0 && (
              <p className="text-sm text-muted-foreground">
                Nenhum evento recente com este número.
              </p>
            )}
            {d.webhookEvents.map((e: any) => (
              <div
                key={e.id}
                className="text-xs border-t border-border py-1.5 flex justify-between gap-2"
              >
                <span>
                  {fmtDate(e.received_at)} · {e.event_kind}
                </span>
                <DsBadge
                  size="sm"
                  intent={
                    e.process_status === "processed" ? "success" : e.error ? "danger" : "neutral"
                  }
                >
                  {e.process_status}
                </DsBadge>
          ...
          <DsCard>
            ...
            {d.queue.map((it: any) => (
              <div key={it.id} className="text-xs border-t border-border py-1.5">
                Fila: <b>{it.status}</b> · tentativas {it.attempts} · {it.last_error ?? "sem erro"}
              </div>
            ))}
      ...
      {events.map((e) => (
        <li key={e.id} className="border-t border-border py-1.5">
          <div className="flex justify-between gap-2">
            ...
            <span className="text-muted-foreground whitespace-nowrap">{fmtDate(e.created_at)}</span>
          </div>
          ...
          {e.external_ids && Object.keys(e.external_ids).length > 0 && (
            <div className="font-mono text-[11px]">{JSON.stringify(e.external_ids)}</div>
          )}
          {e.detail?.error && (
            <div className="text-destructive break-all">{String(e.detail.error)}</div>
          )}
        </li>
      ))}
    </ol>
...
function Reconciliacao({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const m = useMutation({
    mutationFn: (args: { dryRun: boolean; syncPloomes?: boolean }) =>
      run({ data: { ...args, limit: 20, offset } }),
    onSuccess: (r) => {
      setResult(r);
      qc.invalidateQueries({ queryKey: ["leads"] });
      toast.success(r.dryRun ? "Simulação concluída (nada foi gravado)" : "Reconciliação aplicada");
    },
    onError: (e: any) => toast.error(e.message),
  });
  const dups = useQuery({
  ...
  const cons = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        r.dryRun
          ? `Simulação: ${r.groups} grupo(s), ${r.plan.reduce((a: number, p: any) => a + p.mark.length, 0)} seriam marcados`
          : `${r.merged} duplicado(s) marcados (nada apagado)`,
      );
      qc.invalidateQueries({ queryKey: ["leads"] });
    },
    onError: (e: any) => toast.error(e.message),
  ...
  return (
    <div className="space-y-4">
      <DsCard>
        <DsCardHeader
          title="Conversas da Liz → leads"
          subtitle="Reprocessa conversas atendidas pela Liz e cria/atualiza leads só com o que o cliente informou. Simule primeiro."
        />
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <DsButton
            emphasis="outline"
            onClick={() => m.mutate({ dryRun: true })}
            disabled={m.isPending}
          >
            Simular 20
          </DsButton>
          <DsButton onClick={() => m.mutate({ dryRun: false })} disabled={m.isPending}>
            Aplicar (só CRM interno)
          </DsButton>
          <DsButton
            onClick={() => m.mutate({ dryRun: false, syncPloomes: true })}
            disabled={m.isPending}
          >
            Aplicar + enviar qualificados ao Ploomes
          </DsButton>
        ...
        {result && (
          <div className="mt-3 space-y-2">
            <div className="text-sm flex flex-wrap gap-3">
              <span>
                Analisadas <b>{result.totals.analisadas}</b>
              </span>
              <span>
                Criar <b>{result.totals.criar}</b>
              </span>
              <span>
                Atualizar <b>{result.totals.atualizar}</b>
              </span>
              <span>
                Sem dados <b>{result.totals.sem_dados}</b>
              </span>
              <span>
                Ignoradas <b>{result.totals.ignorar}</b>
              </span>
              <span>
                Erros <b>{result.totals.erros}</b>
              </span>
              <span>
                Ploomes <b>{result.totals.enviados_ploomes}</b>
              </span>
              {result.dryRun && (
                <DsBadge intent="info" size="sm">
                  simulação
                </DsBadge>
              )}
            </div>
              ...
              <table className="w-full text-xs min-w-[760px]">
                <thead className="text-muted-foreground">
                  <tr className="text-left">
                    ...
                    <th className="px-2 py-1">Preenchidos</th>
                    <th className="px-2 py-1">Pendentes</th>
                    <th className="px-2 py-1">Ploomes</th>
                  </tr>
                </thead>
                <tbody>
                  {result.rows.map((r: any) => (
                    <tr
                      ...
                      className="border-t border-border hover:bg-accent/40 cursor-pointer"
                      onClick={() => r.lead_id && onOpen(r.lead_id)}
                    >
                      ...
                      <td className="px-2 py-1">
                        {STATUS_LABEL[r.status_previsto] ?? r.status_previsto}
                      </td>
      ...
      <DsCard>
        <DsCardHeader
          title="Duplicados"
          subtitle="Nada é apagado. No CRM interno, os repetidos são marcados como duplicados do registro principal; no Ploomes só é gerado o relatório."
        />
        <div className="flex flex-wrap gap-2">
          <DsButton emphasis="outline" onClick={() => dups.refetch()} disabled={dups.isFetching}>
            Gerar relatório
          </DsButton>
          <DsButton emphasis="outline" onClick={() => cons.mutate(true)} disabled={cons.isPending}>
            Simular consolidação interna
          </DsButton>
          <DsButton
            onClick={() => {
              if (confirm("Marcar duplicados internos (sem apagar)?")) cons.mutate(false);
            }}
            disabled={cons.isPending}
          >
            Consolidar interno
          </DsButton>
        </div>
        {dups.data && (
          <div className="grid md:grid-cols-2 gap-3 mt-3 text-sm">
                ...
                {dups.data.internal.map((g: any) => (
                  <div key={g.phone} className="border border-border rounded p-2 text-xs">
                    ...
                    {g.leads.map((l: any) => (
                      ...
                      >
                        {fmtDate(l.created_at)} · {l.nome || "Não informado"} ·{" "}
                        {l.origem_principal ?? l.origem}
                        {l.ploomes_deal_id ? ` · negócio #${l.ploomes_deal_id}` : ""}
                      </button>
                    ))}
                  </div>
                ))}
            ...
            <div>
              <div className="font-medium mb-1">
                Ploomes: {dups.data.ploomes.groups?.length ?? 0} telefone(s) com contatos repetidos{" "}
                {dups.data.ploomes.error && (
                  <span className="text-destructive">({dups.data.ploomes.error})</span>
                )}
              </div>
              <div className="max-h-72 overflow-auto space-y-1">
                  ...
                  <div key={g.phone_e164} className="border border-border rounded p-2 text-xs">
                    ...
                    {g.contacts.map((c: any) => (
                      <div key={c.Id} className="text-muted-foreground">
                        #{c.Id} · {c.Name} · {fmtDate(c.CreateDate)} · negócios abertos:{" "}
                        {c.deals.length}
                      </div>
                    ))}
                  </div>
...
function Regras() {
  ...
  const f =
    ...
    (q.data
      ? {
          minFatura: q.data.minFatura,
          exigirFatura: q.data.exigirFatura,
          defaultOwnerId: q.data.defaultOwnerId ? String(q.data.defaultOwnerId) : "",
        }
      : null);
  const m = useMutation({
    mutationFn: () =>
      save({
        data: {
          minFatura: f!.minFatura,
          exigirFatura: f!.exigirFatura,
          defaultOwnerId: f!.defaultOwnerId ? Number(f!.defaultOwnerId) : null,
        },
      }),
    onSuccess: () => {
  ...
  return (
    <DsCard className="max-w-xl">
      <DsCardHeader
        ...
        subtitle="Editáveis sem código. Só administração/coordenação salva."
      />
      <div className="space-y-3">
        <div>
          ...
          <Input
            type="number"
            value={f.minFatura}
            onChange={(e) => setForm({ ...f, minFatura: Number(e.target.value) })}
          />
          <p className="text-xs text-muted-foreground mt-1">
            Abaixo disso o lead é desqualificado e não vai ao Ploomes.
          </p>
        </div>
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={f.exigirFatura}
            onChange={(e) => setForm({ ...f, exigirFatura: e.target.checked })}
          />{" "}
          Exigir foto/PDF da fatura para qualificar
        </label>
        <div>
          <Label>Responsável padrão no Ploomes (ID do usuário)</Label>
          <Input
            value={f.defaultOwnerId}
            onChange={(e) => setForm({ ...f, defaultOwnerId: e.target.value })}
            placeholder="ex.: 60022664"
          />
        </div>
        <div className="text-xs text-muted-foreground space-y-1">
          <p>
            Obrigatórios para qualificar: nome, telefone válido, cidade e valor médio da fatura.
            Tipo de ligação, interesse e consentimento ficam como pendências informativas quando não
            citados.
          </p>
          <p>Nada é presumido: campo não informado fica vazio e aparece como pendência.</p>
        </div>
        <DsButton onClick={() => m.mutate()} disabled={m.isPending}>
          {m.isPending ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : (
  ...
  const m = useMutation({
    ...
    onSuccess: (r: any, dryRun) => {
      if (r.ok)
        toast.success(
          dryRun
            ? `Simulação: ${r.createdContact ? "criaria contato" : `reutilizaria contato #${r.contactId}`} · ${r.createdDeal ? "criaria negócio" : `atualizaria negócio #${r.dealId}`}`
            : `Enviado: contato #${r.contactId} · negócio #${r.dealId}`,
        );
    ...
    () =>
      L
        ? [
            ...
            ["E-mail", L.email],
            ["CPF/CNPJ", L.cpf_cnpj],
            ["Cidade", L.cidade ? `${L.cidade}${L.estado ? `/${L.estado}` : ""}` : null],
            ["Fatura média", L.valor_conta_num != null ? money(L.valor_conta_num) : L.valor_conta],
            ["Tipo de ligação", L.padrao_eletrico],
            ["Segmento", L.segmento],
            ["Interesse", L.produto_interesse],
            ["Fatura anexada", L.fatura_url ? "Sim" : null],
            ["Origem", L.origem_principal || L.origem],
            ["Canal", L.canal],
            ["Sistema de entrada", L.sistema_entrada],
            ["Campanha", L.campanha || L.utm_campaign],
            ["Qualificado por", L.qualificado_por],
            ["Qualificado em", L.qualificado_em ? fmtDate(L.qualificado_em) : null],
            ["Consentimento LGPD", L.consent_lgpd == null ? null : L.consent_lgpd ? "Sim" : "Não"],
            ["Etapa", L.stage],
            ["Unidade", L.unit],
          ]
        : [],
    ...
    <Dialog open={!!id} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        ...
        {L && (
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 items-center">
              <DsBadge intent={STATUS_INTENT[L.qualificacao_status] ?? "neutral"}>
                {STATUS_LABEL[L.qualificacao_status] ?? L.qualificacao_status}
              </DsBadge>
              <DsBadge intent={SYNC_INTENT[L.ploomes_sync_status] ?? "neutral"}>
                {SYNC_LABEL[L.ploomes_sync_status] ?? L.ploomes_sync_status}
              </DsBadge>
              ...
              <div className="ml-auto flex gap-2">
                ...
                <DsButton
                  ...
                  disabled={m.isPending || !!L.duplicado_de}
                >
                  {m.isPending ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                  ...
                  {L.ploomes_deal_id ? "Reenviar / atualizar" : "Enviar ao Ploomes"}
                </DsButton>
              </div>
            </div>
            {L.qualificacao_motivo && (
              <p className="text-sm text-muted-foreground">{L.qualificacao_motivo}</p>
            )}
            {Array.isArray(L.campos_pendentes) && L.campos_pendentes.length > 0 && (
              <div className="rounded-lg border border-warning/40 bg-warning/10 p-3 text-sm">
                ...
                <ul className="list-disc pl-5 mt-1">
                  {L.campos_pendentes.map((p: string) => (
                    <li key={p}>{p}</li>
                  ))}
                </ul>
              </div>
            )}
            {L.ploomes_sync_error && (
              <p className="text-sm text-destructive break-all">
                Erro Ploomes: {L.ploomes_sync_error}
              </p>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 text-sm">
              {fields.map(([k, v]) => (
                <div key={k as string}>
                  ...
                  <div className={cn(!v && "text-muted-foreground italic")}>
                    {v || "Não informado"}
                  </div>
                </div>
              ))}
            ...
            <div className="text-xs text-muted-foreground font-mono grid md:grid-cols-2 gap-1">
              ...
              <div>
                Ploomes contato: {L.ploomes_contact_id ?? "—"} · negócio: {L.ploomes_deal_id ?? "—"}
              </div>
              ...
              <div>
                Criado {fmtDate(L.created_at)} · atualizado {fmtDate(L.updated_at)}
              </div>
            ...
            {d!.duplicates.length > 0 && (
              <div className="text-sm">
                <div className="font-medium">Duplicados vinculados a este lead</div>
                {d!.duplicates.map((x: any) => (
                  <div key={x.id} className="text-xs text-muted-foreground">
                    {fmtDate(x.created_at)} · {x.nome || "Não informado"} ·{" "}
                    {x.origem_principal ?? "—"}
                  </div>
                ))}
              </div>
            ...
            <div>
              <div className="font-medium text-sm mb-1">
                Conversa ({d!.messages.length} mensagens)
              </div>
              {d!.messages.length === 0 ? (
                <p className="text-sm text-muted-foreground">Sem conversa vinculada.</p>
              ) : (
            ...
            <div>
              ...
              {d!.queue.length === 0 ? (
                <p className="text-xs text-muted-foreground">Nunca enfileirado.</p>
              ) : (
                d!.queue.map((it: any) => (
                  <div key={it.id} className="text-xs border-t border-border py-1">
                    <b>{SYNC_LABEL[it.status] ?? it.status}</b> · tentativas {it.attempts}/
                    {it.max_attempts} · próxima {fmtDate(it.next_attempt_at)} · {it.reason}
