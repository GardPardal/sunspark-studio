...
function BIModule() {
  ...
  const q = useQuery<PerfilBI>({
    queryKey: ["mod_bi_perfil"],
    queryFn: () => fn() as any,
    staleTime: 30_000,
  });
      ...
      {q.data && (
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Kpi
              label="Conversão"
              value={`${q.data.taxa_conversao_pct.toFixed(1)}%`}
              tone="emerald"
            />
            <Kpi label="Agenda hoje" value={q.data.agenda_hoje} />
            <Kpi
              label="Atrasadas"
              value={q.data.agenda_atrasada}
              tone={q.data.agenda_atrasada > 0 ? "amber" : undefined}
            />
          </div>
              ...
              {q.data.scope === "global" && (
                <>
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full bg-muted px-3 py-1.5 text-xs font-semibold hover:bg-muted/70"
                  >
                    <ExternalLink className="h-3.5 w-3.5" /> Hub Marketing
                  </Link>
                  <Link
                    to="/coordenacao"
                    className="inline-flex items-center gap-1.5 rounded-full bg-muted px-3 py-1.5 text-xs font-semibold hover:bg-muted/70"
                  >
                    <ExternalLink className="h-3.5 w-3.5" /> Coordenação
                  </Link>
                </>
              )}
            </div>
