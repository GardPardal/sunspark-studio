...
function Page() {
  ...
  const q = useQuery({ queryKey: ["radar_overview"], queryFn: () => overview() as any, refetchInterval: 60_000 });
  const d: any = q.data;
  ...
  const mScan = useMutation({
    ...
    onSuccess: (r: any) =>
      toast.success(r?.paused ? "Descoberta pausada nas regras." : `${r.novos ?? 0} itens novos • ${r.relevantes ?? 0} pautas na fila`),
  ...
  return (
    <ModuleShell title="Radar Editorial" subtitle="A redação digital da LZ7: descobre pautas, apura e escreve" active="admin">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <Button onClick={() => mScan.mutate()} disabled={mScan.isPending} className="gap-2">
            <Radar className="h-4 w-4" /> {mScan.isPending ? "Varrendo fontes..." : "Buscar pautas agora"}
          </Button>
        ...
        {tab === "pautas" ? (
          <DsCard className="p-0">
            <div className="p-4">
              <DsCardHeader title="Pautas detectadas" subtitle="Ordenadas por relevância para o público da LZ7" />
            </div>
            <div className="divide-y divide-border/60">
              {(d?.topics ?? []).length === 0 ? (
                <p className="p-6 text-sm text-muted-foreground">
                  Nenhuma pauta ainda. Cadastre as fontes na aba “Fontes” e clique em “Buscar pautas agora”.
                </p>
              ) : null}
              {(d?.topics ?? []).map((t: any) => (
                <div key={t.id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      <DsBadge intent={STATUS_TONE[t.status] ?? "neutral"} size="sm">{t.status}</DsBadge>
                  ...
                  <div className="flex shrink-0 gap-2">
                    {t.status === "identificada" ? (
                      <>
                        <Button size="sm" onClick={() => mAction.mutate({ kind: "approve", id: t.id })} className="gap-1">
                          <Check className="h-3.5 w-3.5" /> Produzir
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => mAction.mutate({ kind: "ignore", id: t.id })}>
        ...
        {tab === "revisao" ? (
          <DsCard className="p-0">
            <div className="p-4">
              <DsCardHeader title="Artigos aguardando revisão" subtitle="Leia, ajuste no CMS e publique" />
            </div>
            <div className="divide-y divide-border/60">
              {(d?.revisao ?? []).length === 0 ? (
                ...
                <div key={p.id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      qualidade {p.quality_score ?? "—"} • {(p.sources ?? []).length} fonte(s) citadas
                    </p>
                  </div>
                  <div className="flex shrink-0 gap-2">
                    <a
          ...
          <DsCard className="p-0">
            <div className="p-4">
              <DsCardHeader
                ...
                action={
                  <Button size="sm" variant="outline" onClick={() => mAction.mutate({ kind: "seed", id: "" })}>
                ...
                <div key={s.id} className="flex items-center justify-between gap-3 p-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium">
                      {s.nome}{" "}
                      <span className="text-xs text-muted-foreground">({s.tipo} • autoridade {s.autoridade})</span>
                    </p>
                    <p className="truncate text-xs text-muted-foreground">
                      {s.dominio} • verificada{" "}
                      {s.ultima_verificacao ? new Date(s.ultima_verificacao).toLocaleString("pt-BR") : "nunca"}
        ...
        {tab === "regras" ? (
          <DsCard>
            <DsCardHeader title="Regras da redação" subtitle="Controle o que o motor pode publicar sozinho" />
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm">
                <span className="mb-1 block font-medium">Confiança mínima para publicar (0-100)</span>
                <input
                  type="number"
                  min={0}
                  max={100}
            ...
            <div className="mt-6">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                {(d?.runs ?? []).map((r: any, i: number) => (
                  <p key={i}>
                    {new Date(r.created_at).toLocaleString("pt-BR")} — {r.tipo}: {r.itens_encontrados} itens,{" "}
                    {r.pautas_novas} novos, {r.pautas_relevantes} na fila, {r.erros} erro(s) em{" "}
                    {Math.round((r.duracao_ms ?? 0) / 1000)}s
                  </p>
                ))}
              </div>
            </div>
