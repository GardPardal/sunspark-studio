...
const brl = (n: number | null | undefined) => n == null ? "—" : n.toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 2 });
function firstOfMonth() { const d = new Date(); return new Date(d.getFullYear(), d.getMonth(), 1).toISOString().slice(0, 10); }
function todayISO() { return new Date().toISOString().slice(0, 10); }

const unitLabel = (u: string) => u === "londrina" ? "Londrina" : u === "ponta_grossa" ? "Ponta Grossa" : u === "wenceslau_braz" ? "Wenceslau Braz" : u;
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <Input type="number" min={0} max={100} value={margem} onChange={(e) => setMargem(Number(e.target.value) || 0)} className="w-28" />
          </div>
          <Button variant="outline" size="sm" onClick={() => { setFrom(firstOfMonth()); setTo(todayISO()); }}>Mês atual</Button>
        </div>
      </Card>
      ...
      {q.data && (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Kpi label="ROAS" value={q.data.roas != null ? `${q.data.roas.toFixed(2)}x` : "—"} tone="emerald" />
            <Kpi label={`Margem est. (${q.data.margem_pct}%)`} value={brl(q.data.margem_estimada_brl)} tone="emerald" />
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {q.data.vendas_por_unidade.length === 0 && (
                    <tr><td colSpan={5} className="px-3 py-6 text-center text-muted-foreground">Sem vendas no período.</td></tr>
                  )}
                  {q.data.vendas_por_unidade.map((u) => (
                    <tr key={u.unit} className="border-t">
                      ...
                      <td className="px-3 py-2 text-right">{brl(u.count > 0 ? u.total / u.count : null)}</td>
                      <td className="px-3 py-2 text-right">{q.data!.receita > 0 ? `${((u.total / q.data!.receita) * 100).toFixed(1)}%` : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card className="p-4 text-xs text-muted-foreground">
            Receita combina vendas do CRM (leads em stage <b>venda</b>/<b>faturado</b>) e vendas manuais registradas. Gasto Ads vem de <b>meta_insights_daily</b>. Margem é estimativa — ajuste o percentual acima conforme sua realidade.
          </Card>
        </>
      )}
    </ModuleShell>
