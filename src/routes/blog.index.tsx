...
function Page() {
  ...
  const filtered = useMemo(
    () =>
      cat
        ? posts.filter((p) => (p.category_id ? catById[p.category_id]?.slug === cat : false))
        : posts,
    [posts, cat, catById],
  );

  const maisLidos = useMemo(
      ...
      <PublicLayout>
        <div className="mx-auto max-w-[1320px] px-4 py-16 md:px-8">
          <EmptyState
            ...
            description="Estamos preparando os primeiros conteúdos. Volte em breve."
          />
        </div>
      </PublicLayout>
    );
  ...
  return (
    <PublicLayout>
      {featured ? (
        <BlogHero
          post={featured}
          categoryName={featured.category_id ? catById[featured.category_id]?.name : undefined}
        />
      ) : (
        <section className="bg-navy-deep text-white">
          <div className="mx-auto max-w-[1320px] px-4 py-12 md:px-8">
            ...
            <p className="mt-3 text-white/70">
              {categories.find((c) => c.slug === cat)?.name ?? "Todos os conteúdos"}
            </p>
          </div>
        </section>
      )}
          ...
          <div className="min-w-0">
            <div className="mb-6 flex items-center justify-between gap-4">
              ...
              {cat ? (
                <button
                  type="button"
                  onClick={() => setCat(null)}
                  className="text-sm font-semibold text-lzgreen-strong hover:underline"
                >
                  Limpar filtro
                </button>
              ) : null}
            ...
            {grid.length ? (
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  {grid.map((p) => (
                    <BlogCard
                      key={p.id}
                      post={p}
                      categoryName={p.category_id ? catById[p.category_id]?.name : undefined}
                    />
                  ))}
                </div>
                  ...
                  <div className="mt-6 grid gap-6 sm:grid-cols-2" aria-hidden>
                    {Array.from({ length: 2 }).map((_, i) => (
                      <div key={i} className="overflow-hidden rounded-2xl border border-border">
                        <DsSkeleton
                          variant="block"
                          className="aspect-[16/10] h-auto w-full rounded-none"
                        />
              ...
              <EmptyState
                ...
                description="Escolha outra categoria para continuar lendo."
              />
            )}
          </div>
