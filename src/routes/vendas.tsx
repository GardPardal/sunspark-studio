  ...
  head: () => ({
    meta: [
      { title: "Vendas & Faturamento — Controle interno" },
      {
        name: "description",
        content:
          "Cadastro e controle interno das vendas faturadas, recebimentos e método de pagamento da LZ7 Energia.",
      },
      { property: "og:title", content: "Vendas & Faturamento — Controle interno" },
      {
        property: "og:description",
        content: "Vendas, faturamento, recebido e a receber em um só lugar.",
      },
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-3 sm:px-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-[11px] text-muted-foreground">
            Portal financeiro — controle interno
          </p>
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => doChange.mutate()}
              disabled={doChange.isPending || !current || next.length < 6}
            >
      ...
      <div className="mx-auto max-w-7xl space-y-4 px-3 py-4 sm:px-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Kpi
            ...
            hint={`${kpis.count} vendas`}
            tone="primary"
          />
          <Kpi
            label="Recebido"
            value={brl(kpis.recebido)}
            hint={`${kpis.pctRecebido.toFixed(1)}% do contratado`}
            tone="emerald"
          />
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative min-w-[220px] flex-1">
              ...
              <Input
                ...
                placeholder="Buscar vendedor, projeto, cidade ou pagamento"
                className="pl-9"
              />
            </div>
        ...
        {query.isLoading && (
          <Card className="p-5 text-sm text-muted-foreground">Carregando vendas…</Card>
        )}
        {query.error && (
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t align-top hover:bg-muted/30">
                    ...
                    <td className="px-3 py-2 text-center">
                      <span
                        className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${r.faturado ? "bg-emerald-500/15 text-emerald-700" : "bg-muted text-muted-foreground"}`}
                      >
                        {r.faturado ? "SIM" : "NÃO"}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (confirm("Excluir esta venda?")) del.mutate(r.id);
                          }}
                          aria-label="Excluir"
                        >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Projeto" className="sm:col-span-2">
              <Input
                ...
                placeholder="WB260360COL - NOME (13.2 KWp)"
              />
            </Field>
            <Field label="Cidade">
              <Input
            ...
            <Field label="Método de pagamento">
              <Input
                ...
                placeholder="SOLFACIL, RECURSO PROPRIO…"
                list="metodos"
              />
              <datalist id="metodos">
                {[
            ...
            <Field label="Recebido (R$)">
              <Input
                ...
                step="0.01"
                value={form.recebido}
                onChange={(e) => setForm({ ...form, recebido: e.target.value })}
              />
            </Field>
            <Field label="A receber (R$)">
              <Input
                type="number"
                step="0.01"
                value={form.a_receber}
                onChange={(e) => setForm({ ...form, a_receber: e.target.value })}
              />
            </Field>
            ...
            <Field label="Data de faturamento">
              <Input
                type="date"
                value={form.faturado_em}
                onChange={(e) => setForm({ ...form, faturado_em: e.target.value })}
              />
            </Field>
            <Field label="Faturado">
              <div className="flex h-9 items-center">
                <Switch
                  ...
                  onCheckedChange={(v) => setForm({ ...form, faturado: v })}
                />
              </div>
            </Field>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => save.mutate()}
              disabled={save.isPending || !form.vendedor || !form.projeto}
            >
