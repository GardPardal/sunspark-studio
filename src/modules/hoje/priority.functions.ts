...
async function getRoles(supabase: any, userId: string) {
  ...
  return Object.fromEntries(roles) as Record<
    "admin" | "coordenador" | "consultor" | "sdr",
    boolean
  >;
}
  ...
  .handler(async ({ context }): Promise<PriorityCard[]> => {
    ...
    if (roles.consultor) {
      ...
      if (nNovos > 0)
        cards.push({
          key: "meus_novos",
          title: `${nNovos} lead(s) novo(s) aguardando você`,
          detail: "Faça o primeiro contato para não perder a janela.",
          count: nNovos,
          severity: nNovos > 5 ? "warning" : "info",
          actionLabel: "Atender agora",
          actionTo: "/crm",
          actionSearch: { stage: "novo", scope: "meus" },
          audience: ["consultor"],
        });
      if (nAtras > 0)
        cards.push({
          key: "meus_atrasados",
          title: `${nAtras} atendimento(s) atrasado(s)`,
          detail: "Deadline de contato já venceu.",
          count: nAtras,
          severity: "error",
          actionLabel: "Resolver agora",
          actionTo: "/crm",
          actionSearch: { filter: "atrasados" },
          audience: ["consultor"],
        });
      if (nHoje > 0)
        cards.push({
          key: "agenda_hoje",
          title: `${nHoje} compromisso(s) na sua agenda hoje`,
    ...
    if (isMgr) {
      const { count: fila } = await supabase
        .from("leads")
        .select("id", { count: "exact", head: true })
        .is("assigned_to", null);
      if ((fila ?? 0) > 0)
        cards.push({
          key: "leads_fila",
          title: `${fila} lead(s) aguardando distribuição`,
          detail: "Rodar a roleta ou distribuir manualmente.",
          count: fila ?? 0,
          severity: (fila ?? 0) > 20 ? "error" : "warning",
          actionLabel: "Distribuir",
          actionTo: "/coordenacao",
      ...
      if ((pend ?? 0) > 0)
        cards.push({
          key: "aprovacoes",
          title: `${pend} solicitação(ões) de acesso pendentes`,
      ...
      void consultantsRaw;
      const { data: profs } = await supabase.from("profiles").select("id").eq("status", "active");
      const consultantIds = (profs ?? []).map((p: any) => p.id);
      if (consultantIds.length) {
        const { data: withAgenda } = await supabase
          .from("agenda_appointments")
          .select("consultor_id")
        ...
        if (semAgenda > 0)
          cards.push({
            key: "consultores_sem_agenda",
            title: `${semAgenda} consultor(es) sem agenda nas próximas 24h`,
      ...
      if ((errs ?? 0) > 0)
        cards.push({
          key: "int_errors",
          title: `${errs} falha(s) de integração nas últimas 24h`,
          detail: "Meta, Ploomes ou e-mail podem estar impactados.",
          count: errs ?? 0,
          severity: (errs ?? 0) > 5 ? "error" : "warning",
    ...
    if (roles.sdr) {
      const { count: naoAtend } = await supabase
        .from("leads")
        .select("id", { count: "exact", head: true })
        .eq("stage", "nao_atendido");
      if ((naoAtend ?? 0) > 0)
        cards.push({
          key: "sdr_reaproveitar",
          title: `${naoAtend} lead(s) não atendido(s) para reaproveitar`,
          detail: "Reengajar via WhatsApp/telefone.",
          count: naoAtend ?? 0,
          severity: "warning",
          actionLabel: "Trabalhar fila",
          actionTo: "/crm",
          actionSearch: { stage: "nao_atendido" },
          audience: ["sdr"],
        });
    }

    // Ordena por severidade
    const rank: Record<PriorityCard["severity"], number> = {
      critical: 0,
      error: 1,
      warning: 2,
      info: 3,
