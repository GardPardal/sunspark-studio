  ...
  .handler(async ({ context }): Promise<SystemHealth> => {
    ...
    const [metaSync, syncLog, emails, leadsToday, campaigns] = await Promise.all([
      ...
      supabase
        ...
        .select("id, provider, status, message, created_at, items_imported")
    ...
    const failed = emailRows.filter(
      (e: any) =>
        (e.status ?? "").toLowerCase().includes("fail") ||
        (e.status ?? "").toLowerCase().includes("error"),
    ).length;
    const integrationErrors24h = logs.filter(
      (l: any) => l.status === "error" || l.status === "partial",
    ).length;
