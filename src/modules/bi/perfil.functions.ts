...
export const getPerfilBI = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<PerfilBI> => {
    ...
    const [{ data: isAdmin }, { data: isCoord }, { data: isSdr }, { data: isCons }] = await Promise.all([
    ...
    const role: PerfilBI["role"] = isAdmin ? "admin" : isCoord ? "coordenador" : isSdr ? "sdr" : isCons ? "consultor" : "user";
    const scope: PerfilBI["scope"] = isAdmin || isCoord ? "global" : "own";

    const first = new Date();
    first.setDate(1); first.setHours(0, 0, 0, 0);

    let leadsQ = supabase.from("leads").select("id, stage, sale_value").gte("created_at", first.toISOString());
    if (scope === "own") leadsQ = leadsQ.eq("assigned_to", userId);
    const { data: leads } = await leadsQ;

    const startToday = new Date(); startToday.setHours(0, 0, 0, 0);
    const endToday = new Date(); endToday.setHours(23, 59, 59, 999);
    let apptQ = supabase.from("agenda_appointments").select("id, starts_at, status").gte("starts_at", startToday.toISOString()).lte("starts_at", endToday.toISOString());
    ...
    const now = Date.now();
    const atrasadas = (appts ?? []).filter((a: any) => a.status === "agendado" && new Date(a.starts_at).getTime() < now).length;
