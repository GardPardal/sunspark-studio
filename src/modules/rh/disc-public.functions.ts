...
export const getDiscInvite = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const [{ data: version }, { data: application }] = await Promise.all([
      supabaseAdmin
        .from("disc_versions")
        .select("id,name,version,instructions,status")
        .eq("id", invite.version_id)
        .maybeSingle(),
      supabaseAdmin
    ...
    const { data: options } = qIds.length
      ? await supabaseAdmin
          .from("disc_options")
          .select("id,question_id,label,ordem")
          .in("question_id", qIds)
          .order("ordem")
      : { data: [] as any[] };
...
export const submitDiscInvite = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) =>
    z
      .object({ token: tokenSchema, answers: z.record(z.string().uuid(), z.string().uuid()) })
      .parse(d),
  )
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    if (new Date(invite.expires_at).getTime() < Date.now())
      throw new Error("Este link expirou. Peça um novo ao RH.");
