...
export type AuditReport = {
  ...
  routes: Array<{ path: string; area: string; audience: string[]; status: "active" | "legacy" | "duplicate" }>;
  navigation_map: Array<{ area: "Operação" | "Marketing" | "Inteligência" | "Gestão"; items: Array<{ label: string; to: string; question: string }> }>;
...
const NAVIGATION_MAP: AuditReport["navigation_map"] = [
  {
    ...
    items: [
      ...
      { label: "Distribuição / Roleta", to: "/coordenacao", question: "Como distribuir novos leads?" },
      { label: "Liberação de contas", to: "/admin", question: "Quem precisa ser aprovado?" },
      { label: "Equipe (Consultores/SDR)", to: "/coordenacao", question: "Como está minha equipe?" },
    ],
  },
  {
    area: "Marketing",
    items: [
      { label: "Meta Ads (tempo real)", to: "/mod/marketing", question: "Como estão minhas campanhas?" },
      { label: "Marketing Hub", to: "/marketing-hub", question: "UTMs cruzam com CRM?" },
      { label: "Diagnóstico de campanhas", to: "/mod/marketing", question: "Quais campanhas devo pausar?" },
      { label: "Landing editável", to: "/landing-editavel.html", question: "Preciso editar o site?" },
    ],
  },
  {
    area: "Inteligência",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Auditoria da plataforma", to: "/mod/auditoria", question: "Como está o inventário do sistema?" },
...
const ROUTE_INVENTORY: AuditReport["routes"] = [
  ...
  { path: "/hoje", area: "Operação", audience: ["admin", "coordenador", "consultor", "sdr"], status: "active" },
  { path: "/agenda", area: "Operação", audience: ["consultor", "coordenador", "admin"], status: "active" },
  { path: "/crm", area: "Operação", audience: ["consultor", "sdr", "coordenador", "admin"], status: "active" },
  ...
  { path: "/marketing-hub", area: "Marketing", audience: ["admin", "coordenador"], status: "active" },
  { path: "/liz-studio", area: "Inteligência", audience: ["admin", "coordenador"], status: "active" },
  { path: "/baixar-app", area: "Gestão", audience: ["admin", "coordenador", "consultor", "sdr"], status: "active" },
  ...
  { path: "/mod/marketing", area: "Marketing", audience: ["admin", "coordenador"], status: "active" },
  ...
  { path: "/mod/financeiro", area: "Inteligência", audience: ["admin", "coordenador"], status: "active" },
...
export const getAuditReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<AuditReport> => {
    ...
    const [{ count: diag }, { data: syncLog }, { data: emails }] = await Promise.all([
      supabase.from("system_diagnostics").select("id", { count: "exact", head: true }).in("status", ["open", "acknowledged"]),
    ...
    const errs = Array.from(byProvider.values()).filter((r) => r.status === "error" || r.status === "partial")
    ...
    return {
      ...
      gaps: [
        { kind: "info", message: "Rotina de auditoria ainda depende de scan sob demanda.", suggestion: "Programar cron 15min via pg_cron para /api/public/hooks/diagnostics." },
        { kind: "info", message: "Biblioteca de UI ainda não centralizada.", suggestion: "Criar `src/components/ui-kit/` na próxima rodada." },
      ],
      tech_health: {
